...
<body>
	...
	<ol>
		<li>$.ajax() : 요청에 대한 상세설정을 할때, header, 요청내용설정</li>
	...
	<p>
		...
		[type : 요청방식(get,post) -> string default : get방식임]<br> <!--생략가능함 default : get 방식  -->
		[data : 서버에 요청할때 전송할 데이터 -> Object({key:value,....})]<br> <!--생략가능함-->
		[dataType : 응답데이터 타입에 대한 설정 -> string(json,html,text...)]<br> <!--생략가능함-->
		success : 응답이 완료되고 실행되는 callback함수 status 200일때(성공) 실행하는 함수 -> (data)=>{}<br>
		error : 응답이 완료되고 실행되는 callback함수 status 200이 아닐때(실패) 실행하는 함수 -> (r,m,e)=>{}<br>
		[complete : 응답이 성공,실패되도 무조건실행되는 함수 -> ()=>{}]<br><!--생략가능함-->
	</p>
	...
	<p>
		문자열을 데이터별로 구분할 수 있게 만들어놓은 것!<br>
		, \n ,$ 등으로 구분할 수 있는 문자열<br>
		예) 유병승,19,남,경기도시흥시$최솔,28,여,경기도안양시$<br>
	</p>
			...
			$.ajax({
				...
				success:data=>{
					...
					actors.forEach(e=>{
						...
						const profile=$("<td>").append($("<img>").attr({src:"<%=request.getContextPath()%>/images/"+actor[2],
							"width":50,"height":50}));
