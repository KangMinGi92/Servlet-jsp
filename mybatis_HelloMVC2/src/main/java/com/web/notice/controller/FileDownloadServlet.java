...
public class FileDownloadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		BufferedInputStream bis=new BufferedInputStream(fis); //fis의 속도를 높여주기위한 버퍼
		
		//4. 한글파일명이 깨지지 않도록 인코딩 처리하기
		String fileRename="";
		String header=request.getHeader("user-agent");
		//IE, 그외 브라우저랑 인코딩 처리방식이 달라서 인코딩을 분리해서 처리해줘야함.
		...
		// attachment 는 무조건 다운로드 하는 옵션이고 그외에 index라는 옵션을 주면 인터넷 창에서 바로 열리는 옵션
