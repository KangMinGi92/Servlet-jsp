...
<body>
	<h3>EL표현식 활용하기</h3>
	<p>
		EL <%-- ${} --%>로 내장객체(공용객체)에 저장된 데이터를 페이지에 출력
		servlet에서 request, session에 setAttribute()메소드로 저장한
		데이터를 자동으로 탐색해서 key값을 기준으로 데이터를 가져옴.
		getter메소드를 호출하지 않고 필드명을 작성하면 EL이 자동으로 getter호출해서 데이터 출력해줌.
		형변환을 하지 않아도됨.
	</p>
	<h2>리터럴값 출력하기</h2>
	<p><%="오랜만에 수업 너무 재미있다" %></p>
	...
	<h3>내장객체(공용객체)에 저장된 데이터 출력하기</h3>
	...
	<%
		...
		String email="yoo@yoo.com"; //내장객체에 저장하지 않고 지역변수로 선언한 변수는 EL표현식으로 출력할 수 없다. !!!
	...
	<p>== : ${testData=="admin"} ${testData eq "admin" }</p> <!--문자열도 equals 대신 ==으로 비교 가능하다 !!-->
	...
	%>
	<p>저장된 객체는 key값으로 불러와서 처리가 가능, 필드에 있는 값을 불러올때는
		getter호출하지않고 필드명으로 불러옴. 
	</p>
	<p>저장된 s불러오기 ${s }</p>
	<p>저장된 s 필드값 불러오기 ${s.type } ${s.name } ${s.price } ${s.weight }</p>
	<p>저장된 s2 필드값 불러오기 ${s2.type } ${s2.name } ${s2.price*2 } ${s2.weight }</p>
