...
<body>
	...
	<p>
		c:if<br>
		test : 조건식=>true, false EL표현식으로 작성
		var : 조건식에 결과를 저장하는 변수
	</p>
	...
	<p>java에서 switch문을 사용하는것과 문법은 비슷하지만, 동작하는건 if ~ else if ~ else랑 비슷함</p>
	<c:choose>
		<c:when test="${su>10 }">
			<p>10보다 크다</p>
		</c:when>
	...
	<p>c:forEach태그를 이용</p>
	<p>
		1. 기본반복문 활용 : i값을 순차적으로 출력하는 방법<br>
		2. 리스트, 배열에 저장된 데이터를 출력(forEach)<br>
	</p>
	<ul>c:forEach태그
		<li>begin : 시작번호</li>
		<li>end : 끝번호</li>
		...
		<li>items : 배열, 리스트</li>
		<li>varStatus : 반복문의 정보를 가지고 있는 객체(index, 반복횟수 정보제공(count),첫번째반복문(first),마지막반복문(last))</li>
	</ul>
	<h4>기본반복문 실행</h4>
	<p>1~10까지 출력하기</p>
	<div>
		...
		<ul>
			...
			<c:forEach var="name" items="${names }" varStatus="vs">
				<li>${vs.first} ${vs.last} ${vs.count} ${name }</li>
			</c:forEach>
		</ul>
	</div>
		
		<%
			List<Snack> snack=List.of(Snack.builder().type("초콜릿").name("투유").price(1200).weight(80).build(),
				...
				<c:if test="${not empty snacks }">
					<c:forEach var="s" items="${snacks }" varStatus="vs">
						<c:if test="${s.price>=2000 }">
							<tr style="background-color:${vs.first or vs.last?'magenta':'lightgray'}">
