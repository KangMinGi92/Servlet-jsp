...
//java에서 제공하는 클래스를 이용해서 양방향 암호화를 진행.
//java.crypto패키지, java.security패키지에서 제공하는 클래스를 이용
// 양방향 암호화는 암호화와 복호화를 할 수 있는 key가 있어야 한다.
// 대칭키암호화 : 암호화, 복호화 키가 1개 -> AES방식의 암호화 알고리즘
// 비대칭키암호화 : 암호화키 1개(공개키), 복호화키(개인키) 1개 총 2개가 쌍으로 있음. ->RSA방식의 암호화 알고리즘
...
public class AESEncryptor {
	...
	private String path;// key 파일로 저장된 위치를 나타내는 변수
	
	public AESEncryptor() {
		// key값을 생성하거나 가져오기
		//1. 생성된 키가 있으면 저장된 파일에서 key를 가져와서 변수key에 저장
		//2. 생성된 키가 없으면 SecretKey 클래스를 생성하여 변수key에 저장
	...
	private void generateKey(File keyFile) {
		//java.crypto패키지에서 제공하는 KeyGenerate클래스를
		//이용해서 SecretKey를 생성한다.
		//key 생성시 sault 값이 필요함.
