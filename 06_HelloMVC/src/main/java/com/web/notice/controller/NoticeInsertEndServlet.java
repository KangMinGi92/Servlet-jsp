...
public class NoticeInsertEndServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//2 : 파일을 저장할 위치 -> 절대경로 -> String
		//3 : 업로드파일의 최대크기를 설정 -> int (1024(byte)*1024(MB)*1024(GB)*1024(TB))
		//4 : 인코딩방식 -> String(UTF-8)
		//5 : rename규칙설정(클래스) -> 기본제공클래스(DefaultFileRenamePolicy)
		//*rename규칙을 설정하는 이유 : 파일을 업로드하면 서버에 저장하는데 불특정 다수의 클라이언트가 각각 동일한 이름의 파일을 업로드하면 덮어씌기가
		//							되기때문에 rename규칙을 설정해줘야한다.!
		...
		//원본파일명, 재정의파일명, 파일사이즈 등의 정보를 가져올 수 있다.
